...
if __name__ == "__main__":
    ...
    for target_var, y_col in zip(TARGET_VARS, y_cols):
        try:
            ...
            print("Data was not preprocessed before, so we will do it now...")
        ...
        for representation in representations:
            for horizon in horizons:
                try:
                    with open(
                        f"data/{BASE_ATS_OUT_FILE}_{representation}_hor{horizon}_{target_var}.pkl",
                        "rb",
                    ) as file:
                        ats = pickle.load(file)
                except FileNotFoundError:
                    print("ATS was not yet fitted, so we will fit it now...")
                    ...
                    ats.save(
                        f"data/{BASE_ATS_OUT_FILE}_{representation}_hor{horizon}_{target_var}"
                    )

                for model in models:
                    print(f"Evaluating {model}")
                    ats.finalize(model=model, progress_bar=False)
                    ...
                    for i, event in enumerate(X_test.to_dict(orient="records")):
                        ...
                        y_preds.append(ats.predict_one(event))
                        print_progress_bar(
                            i + 1,
                            len(y_test),
                            prefix="Prediction:",
                    ...
                    with open(
                        f"data/y_preds_{target_var}_{ats.model}_{representation}_hor{horizon}.pkl",
                        "wb",
                    ) as file:
